...
public class Pathfinding : MonoBehaviour {
    ...
    List<Vector3> GetFinalPath(Node a_StartingNode, Node a_EndNode)
    {
        ...
        while(CurrentNode != a_StartingNode)//While loop to work through each node going through the parents to the beginning of the path
        {
            ...
            if (xD != CurrentNode.iGridX - CurrentNode.ParentNode.iGridX || yD != CurrentNode.iGridY - CurrentNode.ParentNode.iGridY)
            {
                CornerNodes.Add(CurrentNode.vPosition);
            }
            xD = CurrentNode.iGridX - CurrentNode.ParentNode.iGridX;
            yD = CurrentNode.iGridY - CurrentNode.ParentNode.iGridY;
